...
public class BoundaryWalkerActor extends ActorBasicJava {
    ...
    private enum State {walking, obstacle, end, paused};
    private IssWsHttpJavaSupport support;
    private State curState       =  State.paused ;
    private State oldState = null;
    private JSONObject oldMsg;

    private int stepNum          = 1;
    private boolean isRobotHalted = true;
...
//Removed since we want use just the fsm, without any 'external' code
    ...
    protected void fsm(String move, String endmove){
        ...
        switch( curState ) {
            ...
            case paused:{
                if(move.equals("RESUME")){
                    ...
                    if(oldState == null) {
                        ...
                        curState = oldState;
                        fsm(oldMsg.getString("move"),oldMsg.getString("endmove") );
                    }
                }
                 break;
            }
            ...
            case walking: {
                if (move.equals("moveForward") &&( endmove.equals("true") || endmove.equals("halted"))) {
                    //curState = State.walk;
            ...
            case obstacle :
                if( move.equals("turnLeft") && (endmove.equals("true") || endmove.equals("halted"))) {
                    if( stepNum < 4) {
                        stepNum++;
    ...
    protected void msgDriven( JSONObject infoJson){
        if( infoJson.has("endmove") ) {
            System.out.println("WWWWWWWWWWWWWWWWWW " + infoJson.toString());
            oldMsg = infoJson;
    ...
    protected void handleRobotCmd( JSONObject robotCmd ){
        ...
        if(cmd.equals("STOP") && !isRobotHalted){
            isRobotHalted = true;
            fsm(cmd, "");
        }else if(cmd.equals("RESUME") && isRobotHalted){
            isRobotHalted = false;
            fsm(cmd, "");
        }
        System.out.println("===================================================="    );
