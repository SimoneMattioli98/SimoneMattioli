...
- a collision between the robot and an obstacle { "collision" : "false", "move": "moveForward"}
    ...
    protected  void handleCommand( JSONObject commandinfo ){
        String command = (String) commandinfo.get("robotcmd");
        switch (command){
            case "RESUME":
                robotBehaviorLogic.doBoundaryGoon();break;

            case "STOP":
                robotBehaviorLogic.haltBoundary();break;

            default:
                System.out.println("RobotInputController | wrong command: "+command);
        }
    }
    ...
    protected void handleEndMove(JSONObject endmove ){
        ...
        switch( answer ){
            ...
            case "halted"     : System.out.println("RobotInputController | handleEndMove to do     " );break;
